...
# Model = joblib.load('G:/FINAL CODE/MACHINE LEARNING/ITPML03-FINAL CODING/Deploy/users/CYBER.pkl')
...
ml_model = joblib.load('users/CYBER.pkl')  # Adjust path if needed
...
def Deploy_8(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            features = np.array([[cleaned[field] for field in cleaned]])
            prediction = ml_model.predict(features)[0]

            label_map = {
                0: 'ARP_poisioning', 1: 'DDOS_Slowloris', 2: 'DOS_SYN_Hping', 3: 'Metasploit_Brute_Force_SSH',
                4: 'MQTT_Publish', 5: 'NMAP_FIN_SCAN', 6: 'NMAP_OS_DETECTION', 7: 'NMAP_TCP_scan',
            ...
            return render(request, 'app/output.html', {
                'predict': result,
                'datas': UserPredictModel.objects.all()
            })
...
# views.py
@csrf_exempt
def get_random_iiot_data(request):
    file_path = os.path.join(settings.BASE_DIR, 'users', 'cyber1.csv')  # Use clean one!
    df = pd.read_csv(file_path)
    random_row = df.sample(n=1).iloc[0]
    
    data = {}
    for key, value in random_row.to_dict().items():
        if pd.isna(value):
            data[key] = 0.0
        else:
            data[key] = float(value) if isinstance(value, (np.integer, np.floating)) else value
    
    # Optionally remove the label so user doesn't see it
    data.pop('Attack_type', None)
